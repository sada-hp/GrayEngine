...
*Depricated function
*/
void DrawableObj::updateUniformBuffer(VkDevice device, uint32_t imageIndex)
{
	static auto startTime = std::chrono::high_resolution_clock::now();
	...
	ubo.model = glm::rotate(glm::mat4(1.0f), time * glm::radians(30.0f), glm::vec3(0.0f, 0.0f, 1.0f));
	ubo.view = glm::lookAt(glm::vec3(2.0f, 2.0f, 2.0f), glm::vec3(0.0f, 0.0f, 0.0f), glm::vec3(0.0f, 0.0f, 1.0f));
	ubo.proj = glm::perspective(glm::radians(45.0f), (float)VulkanAPI::m_getRenderer()->getExtent().width / (float)VulkanAPI::m_getRenderer()->getExtent().height, 0.1f, 10.0f);
	ubo.proj[1][1] *= -1;

	void* data;
	vmaMapMemory(VulkanAPI::m_getRenderer()->getMemAllocator(), uniformBuffer.Allocation, &data);
	memcpy(data, &ubo, sizeof(ubo));
	vmaUnmapMemory(VulkanAPI::m_getRenderer()->getMemAllocator(), uniformBuffer.Allocation);
}